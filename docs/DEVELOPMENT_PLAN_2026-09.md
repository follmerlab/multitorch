...
14. **Cr³⁺ fixture path vs Fortran `.ban_out` (fixed 2026-09-15: the assembler now uses copy k of a triad listed k+1 times in the .ban; before, both PRMULT copies of S1+→S1− were assembled from copy 0, which Mn²⁺/Fe³⁺/Co²⁺ tolerate because their two copies carry equal intensity, and Cr³⁺ does not. Now cosine 0.99999999, and the dipole sum rule `tests/test_spectrum/test_dipole_sum_rule.py` holds for every fixture; found by that invariant, FABLE_HANDOFF_2026-09-15 R14. Original entry follows.) (found 2026-09-13 by the S7 union-window metric).** Phase 5 at the fixture's own reductions agrees with the bootstrap `.ban_out` to cosine ≥ 0.99999997 for Ti⁴⁺, V³⁺, Mn²⁺, Fe²⁺, Fe³⁺, Co²⁺, Ni²⁺ and nid8ct, but Cr³⁺ is at 0.9889 (max|Δ| 0.19 of the peak, area 1.002), independent of T and max_gs and with no ground-state degeneracy (S1+ gap 2 eV). The old test excused it as "degeneracy rounding" at a 0.98 threshold. Observations: the two PRMULT copies of the S1+→S1− triad are assembled from copy 0 (`read_ban` drops the copy index, `_find_transi_block` returns the first match), and phase 5 builds 7770 S1− final states where ttban printed 2730; both also hold for Mn/Fe³⁺/Co (3810 vs 1410), which match, so neither alone explains Cr. Next step: per-triad stick comparison for Cr against the `.ban_out`.
...
- **S6. [done 2026-09-13 except SCA-001 flip and the get_sticks degeneracy item (residual 15)]** API semantics. Document per-element fixture CT defaults or default `lmct=0`; `delta` accepts a scalar Δ (with `u`) or `{'eg2','ef2'}` (the `(Δ, Δ_f)` tuple form was not implemented); implement or remove `u`; fix docstrings; wire or drop `med_energy`; exclude zero-intensity sticks from range/median; replace `xv != 0.0` with `is None`; normalise or document ground-state degeneracy in `get_sticks*`; SCA-001 flip with deprecation warning. 1 day.
