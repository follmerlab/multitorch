...
| S4 D4h CF blocks, S5 Oh transition blocks | done; from-scratch == Fortran end-to-end for Ni d⁸ (integer J) |
| S1a fixture-path exact decomposition (`build_cowan.py`) | **open — next** |
| S6 API semantics, S7 parity metric, S8 FD autograd test | open |
| Half-integer J on the projected emitters (Fe(III), Co(II), Cu(II), …) | open → WP-B double group |
| Fe v0 fits re-run | blocked on S1a |
