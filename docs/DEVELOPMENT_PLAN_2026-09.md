...
18. **Unnormalised Boltzmann weights put a kink in the spectrum at every ground-level crossing (open, found 2026-09-13 during A5).** `get_sticks*` weights by `exp((E_min − E_g)/kT)` without dividing by the partition function, exactly as pyctm (`pyctm/get_spectrum.py:84`). The total weight Z·exp(E_min/kT) is then non-differentiable wherever the lowest ground branch switches, e.g. the D4h Oh limit dt = 0: from scratch, Ni²⁺ at T = 80 K, max_gs = 40, the weighted loss has one-sided slopes +2.41 / −5.55 (normalised to L) and its area peaks at dt = 0. Autograd returns one side, central FD their mean (−1.57); neither is "the" derivative. Dividing by the pool partition function makes both slopes −1.574. **Consequence:** the planned A5 oracle (FD at dt = ds = 0 and at soc = 0) cannot pass on the current forward model; with max_gs = 1 the same points are also discontinuous (residual 15, FD ≈ ±5000). Normalising changes absolute intensities by 1/Z (a global factor per spectrum, spectral shape unchanged) and departs from pyctm: **user decision pending.** Away from exact crossings (dt = ds = 1e-3, soc = 1e-3, Cr³⁺ Oh fixture) autograd already matches FD to ≤ 3.5e-6 with the current `safe_eigh`.
...
| A5 degeneracy-safe eigh backward | **blocked**: exact-crossing oracle is ill-posed on the current forward model (residuals 18, 19) |
...
- **A5 not done**, see residuals 18/19. Decision needed: normalise Boltzmann weights by Z (departs from pyctm absolute intensities) or keep pyctm's convention and test gradients only away from exact level crossings.
